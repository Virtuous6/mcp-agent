...
class SlackMetaAgent:
        ...
        if supabase_project_id:
            try:
                ...
                if os.path.exists(secrets_file):
                    ...
                    with open(secrets_file, "r") as f:
                        ...
                        if "supabase" in secrets:
                            ...
                            self.logger.info(
                                "📄 Loaded Supabase credentials from mcp_agent.secrets.yaml"
                            )
                ...
                if anon_key or service_role_key:
                    ...
                    # Test the direct client and disable logging if it fails
                    try:
                        import asyncio

                        # Quick health check
                        health = asyncio.create_task(
                            self.supabase_direct_client.health_check()
                        )
                        # Don't await here as it would block initialization
                    except Exception as health_error:
                        self.logger.warning(
                            f"⚠️ Direct client health check failed: {health_error}"
                        )
                else:
                    self.logger.warning(
                        "⚠️ Supabase keys not found in secrets file or environment - direct client disabled"
                    )
    ...
    def _is_mcp_server_addition_request(self, message: str) -> bool:
        ...
        # Return true if we have addition indicators and no discovery patterns
        result = (
    ...
    async def _execute_orchestrated_workflow(
        ...
        try:
            ...
            # 🔧 NEW: Handle MCP server addition workflow
            if (
                len(agents) == 1
...
**Please specify the type:**""",
    ...
    async def test_mcp_server_addition(self):
        """Test the MCP server addition functionality"""
        ...
        test_cases = [
            ...
            {
                "message": "add mcp stdio server with command 'python server.py'",
                "description": "Stdio server info",
                "expected_parsed": {
                    "transport": "stdio",
            ...
            {
                "message": "setup mcp websocket server description: 'My awesome API server'",
                "description": "Websocket with description",
                "expected_parsed": {
                    "transport": "websocket",
        ...
        addition_test_cases = [
            ...
            "create mcp server with URL https://api.test.com",
            "setup mcp server",
        ]
        ...
        self.logger.info("📝 Testing message parsing...")
        for test_case in test_cases:
            parsed_info = self._parse_server_info_from_message(test_case["message"])
            expected = test_case["expected_parsed"]

            parsing_success = True
            missing_fields = []

            for field, expected_value in expected.items():
                if field not in parsed_info or parsed_info[field] != expected_value:
                    parsing_success = False
                    missing_fields.append(
                        f"{field}: expected '{expected_value}', got '{parsed_info.get(field, 'missing')}'"
                    )

            if parsing_success:
                results.append(
                    {
                        "test": f"Message parsing: {test_case['description']}",
                        "status": "✅ PASS",
                        "details": f"Parsed: {parsed_info}",
                    }
                )
            else:
                results.append(
                    {
                        "test": f"Message parsing: {test_case['description']}",
                        "status": "❌ FAIL",
                        "details": f"Issues: {', '.join(missing_fields)}",
                    }
                )

        # Test 3: Intent Analysis Integration
        self.logger.info("🤖 Testing intent analysis integration...")
        test_message = "add mcp server"

        try:
            intent_analysis = await self._analyze_user_intent_dynamic(test_message)
            required_agents = intent_analysis.get("required_agents", [])

            if "mcp_server_manager" in required_agents:
                results.append(
                    {
                        "test": "Intent analysis integration",
                        "status": "✅ PASS",
                        "details": f"Correctly routed to mcp_server_manager with strategy: {intent_analysis.get('execution_strategy', 'unknown')}",
                    }
                )
            else:
                results.append(
                    {
                        "test": "Intent analysis integration",
                        "status": "❌ FAIL",
                        "details": f"Expected mcp_server_manager, got agents: {required_agents}",
                    }
                )
        except Exception as e:
            results.append(
        ...
        self.logger.info("📋 Testing agent registry...")
        if "mcp_server_manager" in self.agent_registry:
            agent_spec = self.agent_registry["mcp_server_manager"]
            results.append(
                {
                    "test": "Agent registry",
                    "status": "✅ PASS",
                    "details": f"Agent registered with servers: {agent_spec.server_names}, capabilities: {agent_spec.capabilities}",
                }
            )
        else:
            results.append(
                {
                    "test": "Agent registry",
                    "status": "❌ FAIL",
                    "details": "mcp_server_manager not found in agent registry",
                }
            )

        # Test 5: Database Query Structure (Mock Test)
        self.logger.info("🗄️ Testing database query structure...")
        try:
            test_server_info = {
            ...
            # Test SQL query construction without actually executing
            server_name = test_server_info["server_name"].replace("'", "''")
            display_name = test_server_info["display_name"].replace("'", "''")
            description = test_server_info["description"].replace("'", "''")
            transport = test_server_info["transport"]
            url = test_server_info["url"].replace("'", "''")
            args = json.dumps(test_server_info["args"]).replace("'", "''")

            # Check if query can be constructed without errors
            test_query = f"""
            INSERT INTO mcp_servers (
                configuration_id,
                server_name,
            ...
            # If we got here without errors, query structure is valid
            results.append(
                {
                    "test": "Database query structure",
                    "status": "✅ PASS",
                    "details": "SQL query constructed successfully with proper escaping",
                }
            )
            ...
            results.append(
                {
                    "test": "Database query structure",
                    "status": "❌ FAIL",
                    "details": f"SQL construction error: {str(e)}",
                }
            )
...
1. Message: `@meta-agent add mcp server`
2. Follow the interactive prompts
3. Provide server details when asked
4. Confirm the server is added to database

**Example usage:**
`@meta-agent add mcp server called 'my_api' with URL https://api.example.com/mcp using sse transport`
"""
